...
find "/etc" -writable -print0 | xargs -0 -I{} -P64 -n1 chown -f "${current_uid}":"${current_gid}" "{}"
find "/usr/share/nginx" -writable -print0 | xargs -0 -I{} -P64 -n1 chown -f "${current_uid}":"${current_gid}" "{}"
find "/var" -writable -print0 | xargs -0 -I{} -P64 -n1 chown -f "${current_uid}":"${current_gid}" "{}"
